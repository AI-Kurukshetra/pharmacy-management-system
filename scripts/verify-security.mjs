...
async function main() {
  const home = await fetch(`${BASE_URL}/dashboard`, { redirect: "manual" });
  ...
  assert("security header: csp", csp.includes("default-src"));
  assert("security header: hsts", hsts.toLowerCase().includes("max-age"));
  assert("security header: x-frame-options", xfo.toUpperCase() === "DENY");
  assert("security header: x-content-type-options", xcto.toLowerCase() === "nosniff");

  const stripe = await fetch(`${BASE_URL}/api/integrations/stripe/webhook`, {
    method: "POST",
    headers: { "content-type": "application/json", "stripe-signature": "invalid" },
    body: JSON.stringify({ ping: true }),
  });
  assert("stripe webhook invalid signature rejected", stripe.status === 400, `status=${stripe.status}`);

  const surescripts = await fetch(`${BASE_URL}/api/integrations/surescripts/inbound`, {
    method: "POST",
    headers: { "content-type": "application/json", "x-surescripts-signature": "invalid" },
    body: JSON.stringify({ test: true }),
  });
  assert(
    "surescripts webhook invalid signature rejected",
    surescripts.status === 401 || surescripts.status === 503,
    `status=${surescripts.status}`,
  );
