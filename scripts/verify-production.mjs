...
const BASE_URL = process.env.BASE_URL || "https://pharmacy-platform-coral.vercel.app";
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || "admin@pharmacy-platform.app";
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "ChangeMe123!";
const CASHIER_EMAIL = process.env.CASHIER_EMAIL || "cashier@pharmacy-platform.app";
const CASHIER_PASSWORD = process.env.CASHIER_PASSWORD || "ChangeMe123!";
...
async function login(email, password) {
  ...
  return out.json?.session?.access_token || "";
}
...
async function main() {
  const unauth = await http("/api/patients");
  assert("unauth blocked", unauth.status === 401, JSON.stringify(unauth.json));

  const adminToken = await login(ADMIN_EMAIL, ADMIN_PASSWORD);
  assert("admin login token", Boolean(adminToken));

  const cashierToken = await login(CASHIER_EMAIL, CASHIER_PASSWORD);
  assert("cashier login token", Boolean(cashierToken));

  const patientsAdmin = await http("/api/patients", {
    headers: { authorization: `Bearer ${adminToken}` },
  });
  assert("admin can list patients", patientsAdmin.ok && Array.isArray(patientsAdmin.json?.data));

  const patientsCashier = await http("/api/patients", {
    headers: { authorization: `Bearer ${cashierToken}` },
  });
  assert("cashier blocked from patients", patientsCashier.status === 403, JSON.stringify(patientsCashier.json));

  const txCashier = await http("/api/transactions/daily-summary", {
    headers: { authorization: `Bearer ${cashierToken}` },
  });
  assert("cashier can access transactions", txCashier.ok && typeof txCashier.json?.total_sales === "number");

  const reportsAdmin = await http("/api/reports", {
    headers: { authorization: `Bearer ${adminToken}` },
  });
  assert("reports root contract", reportsAdmin.ok && Array.isArray(reportsAdmin.json?.available_reports));

  const exportCsv = await http("/api/reports/export/csv?report_type=financial&pharmacy_id=11111111-1111-1111-1111-111111111111", {
    headers: { authorization: `Bearer ${adminToken}` },
  });
  assert("csv export signed url", exportCsv.ok && typeof exportCsv.json?.download_url === "string");

  const iot = await http("/api/integrations/iot/reading", {
    method: "POST",
    headers: {
  ...
  assert("iot ingestion", iot.ok && iot.json?.data?.sensor_id === "VERIFY-COLD-1", JSON.stringify(iot.json));
