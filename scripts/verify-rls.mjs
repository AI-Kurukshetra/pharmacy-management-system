...
async function main() {
  assert("env NEXT_PUBLIC_SUPABASE_URL set", Boolean(SUPABASE_URL));
  assert("env NEXT_PUBLIC_SUPABASE_ANON_KEY set", Boolean(ANON_KEY));
  assert("env SUPABASE_SERVICE_ROLE_KEY set", Boolean(SERVICE_ROLE_KEY));
  if (process.exitCode) process.exit(process.exitCode);

  const anonPatients = await rest("patients?select=id&limit=1", ANON_KEY);
  assert("anon cannot read patients table", !anonPatients.ok, `status=${anonPatients.status}`);

  const anonClaims = await rest("claims?select=id&limit=1", ANON_KEY);
  assert("anon cannot read claims table", !anonClaims.ok, `status=${anonClaims.status}`);

  const servicePatients = await rest("patients?select=id&limit=1", SERVICE_ROLE_KEY);
  assert("service role can read patients for maintenance ops", servicePatients.ok, `status=${servicePatients.status}`);

  const serviceInventory = await rest("inventory_items?select=id&limit=1", SERVICE_ROLE_KEY);
  assert("service role can read inventory", serviceInventory.ok, `status=${serviceInventory.status}`);
